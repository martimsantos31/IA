    ...
    while True:
        ...
        try:
            ...
            if "snakes" in state and "food" in state:
                ...
                print("Snake info:",snakes_update)
        ...
        if new_game or not all(
            ...
            for snake in snakes_update:
                ...
                snakes[snake["name"]].range = snake["range"]
        ...
        highlight_surface = pygame.Surface((WIDTH * SCALE, HEIGHT * SCALE), pygame.SRCALPHA)

        # Highlight the snake's vision
        for snake in snakes.values():
            head = snake.body[0]
            vision_range = snake.range

            # Calculate vision area based on Manhattan distance
            neighbors = [
                (x % WIDTH, y % HEIGHT)
                for x in range(head[0] - vision_range, head[0] + vision_range + 1)
                for y in range(head[1] - vision_range, head[1] + vision_range + 1)
                if abs(head[0] - x) + abs(head[1] - y) <= vision_range
            ]

            # Highlight the vision area
            for neighbor in neighbors:
                pygame.draw.rect(
                    highlight_surface,
                    (200, 200, 0, 128),  # Semi-transparent yellow
                    pygame.Rect(neighbor[0] * SCALE, neighbor[1] * SCALE, SCALE, SCALE),
                )

        display.blit(highlight_surface, (0,0))
